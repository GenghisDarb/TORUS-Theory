...
def getSpaceCoordinates(space_name):
    if (
        space_name[2:4].isdigit()
        and space_name[5:7].isdigit()
        and space_name[9:11].isdigit()
        and space_name[12:14].isdigit()
    ):
        return np.array(
            [
                int(space_name[2:4]),
...
def GetNextCor(space_name, comp_name, L, cor):
    ...
    space_dirX = -np.sign(
        int(np.array([space_cors[2], space_cors[0]]).max() <= comp_cors[0]) - 0.5
    ) * int(space_name[1] == "X")
    space_dirY = np.sign(
        int(np.array([space_cors[3], space_cors[1]]).min() >= comp_cors[1]) - 0.5
    ) * int(space_name[1] == "Y")
    ...
    for other_node in other_nodes:
        if other_node.isConnected() and other_node.name not in node_stack:
            ...
            if np.sum(next_cor - cor) > 0:
                space_cors.append(
                    {"spacename": space.name, "cor": cor, "nextcor": next_cor}
                )
            else:
                space_cors.append(
                    {"spacename": space.name, "cor": next_cor, "nextcor": cor}
                )
...
def DrawSpace(space_cors, space_power, space_len, canvas, disablelabels=False):
    [cor, next_cor, spacename] = [
        space_cors["cor"],
        space_cors["nextcor"],
        space_cors["spacename"],
    ]
...
def DrawSpaceText(space_cors, space_power, space_len, canvas, fontsize=8):
    ...
    if space_cors["spacename"][1] == "X":
        ...
        textobject.setTextOrigin(
            (cor[0] + next_cor[0] - stringWidth(txt, "Times-Roman", fontsize) / inch)
            * inch
            / 2,
            cor[1] * inch - linewidth / 2 - fontsize * 1.5,
        )
    ...
    if space_cors["spacename"][1] == "Y":
        textobject.setTextOrigin(
            cor[0] * inch
            - linewidth / 2
            - stringWidth(txt, "Times-Roman", fontsize)
            - 6,
            (cor[1] + next_cor[1] - fontsize * 1.5 / inch) * inch / 2,
        )
    #        canvas.rotate(-90)
...
def DrawComp(kat, comp_cors, canvas, comp_dict, old_new_dict={}, disablelabels=False):
    ...
    cor = np.array(
        [cor[0] * inch - xsign * half_xdim, cor[1] * inch - ysign * half_ydim]
    )
...
def DrawCompText(kat, comp_cors_more, canvas, fontsize=8, old_new_dict={}):
    ...
    if comp_name[0] == "L":
        textobject.textLine(
            text=str(int(comp.P.value * 100) / 100)
            + "W, "
            + str(int(comp.phase.value))
            + "deg"
        )

    if comp_name[0] == "Q":
        textobject.textLine(
            text=str(int(comp.db.value)) + "dB, " + str(int(comp.phase.value)) + "deg"
        )
...
def FindOrientation(kattemp, comp_name, space_powers):
    ...
    comp = kattemp.components[comp_name]
    comp_nodes = np.array(kattemp.nodes.getComponentNodes(comp))
    nodes_comps = np.array(
        [np.array(kattemp.nodes.getNodeComponents(node)) for node in comp_nodes]
    )
    comp_spaces_names = np.array(
        [
            comps[comps != comp][0].name if comps[comps != comp][0] is not None else ""
            for comps in nodes_comps
        ]
    )
    ...
    connected_comp_spaces_names = np.array(
        [comps[comps != comp][0].name for comps in connected_nodes_comps]
    )
    ...
    if comp_name[0] in ("B", "F"):
        ...
        if BS_inds:
            ...
            for space_name in comp_spaces_names[BS_inds]:
                handedness = handedness * np.sign(
                    SpaceCompFlipFactor(space_name, comp_name) - 0.5
                )
            ...
            if comp_name[0] == "F":
                if BSFangle == 0:
                    if (
                        comp_spaces_names[BS_inds[0]][1] == "X"
                        and not SpaceCompFlipFactor(
                            comp_spaces_names[BS_inds[0]], comp_name
                        )
                    ) or (
                        comp_spaces_names[BS_inds[0]][1] == "Y"
                        and SpaceCompFlipFactor(
                            comp_spaces_names[BS_inds[0]], comp_name
                        )
                    ):
                        flipAngle = 180
    ...
    if comp_name[0] in ["M", "L", "Q", "P"]:
        ...
        return SpaceCompFlipFactor(chosen_space, comp_name) * 180 + 90 * int(
            chosen_space[1] == "Y"
        )
...
def ComputeSpacePowers(kat_):
    ...
    isspace = np.array(
        [
            "space" in str(type(katWithDCPDs.components[comp]))
            for comp in katWithDCPDs.components
        ]
    )
    ...
    for spacename in spacenames:
        spacenodes = katWithDCPDs.nodes.getComponentNodes(
            katWithDCPDs.components[spacename]
        )
...
def FixDetectorCode(
    ...
    def findnextcor(dir):
        if dir == "X":
            nextcor = previous_comp_cor + np.array(
                [no_of_grid_points, 0]
            )  # *occupy_sign
        elif dir == "Y":
            nextcor = previous_comp_cor + np.array(
        ...
        for compname in kat_.components:
            ...
            if (nextcor == compcor).all() and (
                replace_text is None or replace_text in kat_.components
            ):
                if dir == "X":
                    nextcor = previous_comp_cor + np.array(
                        [-no_of_grid_points, 0]
    ...
    nodes_comps = np.array(
        [np.array(kat_.nodes.getNodeComponents(node)) for node in detector_space_nodes]
    )
    ...
    for node in previous_comp_nodes[
        previous_comp_nodes != detector_space_nodes[previous_comp_ind]
    ]:
        if node.isConnected():
            ...
            if previous_comp.name[0] in ("B", "F"):
                nodes_index_diff = np.abs(
                    np.where(previous_comp_nodes == node)[0][0]
                    - np.where(
                        previous_comp_nodes == detector_space_nodes[previous_comp_ind]
                    )[0][0]
                )
                if (
                    nodes_index_diff in [1, 3]
                    and space.name[1] == "Y"
                    or nodes_index_diff == 2
                    and space.name[1] == "X"
                ):
                    # occupy_sign = -int(np.sign(int(isOccupied(kattemp, nextcor + [no_of_grid_points, 0]))-0.5))
                    nextcor = findnextcor("X")
    ...
    new_space_name = (
        "S"
        + detector_space_direction
        + FormatCorString(previous_comp_cor)
        + "__"
        + FormatCorString(nextcor)
    )
    ...
    newkattext = "".join(kat_.generateKatScript()).replace(
        detector_space_text, new_space_name
    )

    if replace_text is None:
        for detector_space_node in detector_space_nodes:
            if any(
                [component is None for component in detector_space_nodes[0].components]
            ):
                break
    ...
    for compname, comp in kattemp.components.items():
        if compname[0] == "S":
            ...
            if np.sum(nextcors - cors) < 0:
                new_space_name = (
                    compname[0:2]
                    + FormatCorString(nextcors)
                    + "__"
                    + FormatCorString(cors)
                )
            kat_script = kat_script.replace(
                " " + compname + " ", " " + new_space_name + " "
            )
...
def AlignComponents(comps_struct, spaces_struct):
    ...
    space_nextcors = np.array(
        [space_struct["nextcor"] for space_struct in spaces_struct]
    )

    comps_names = np.array([comp_struct["compname"] for comp_struct in comps_struct])
    grid_comps_cors = np.array(
        [getComponentCoordinates(comp_name) for comp_name in comps_names]
    )
    spaces_names = np.array(
        [space_struct["spacename"] for space_struct in spaces_struct]
    )
    grid_spaces_cors = np.array(
        [getSpaceCoordinates(space_name) for space_name in spaces_names]
    )
...
def ParseUIFOkat(kat_, gridsize, skip_comps=[], old_new_dict={}):
    ...
    for compname, comp in kattemp.components.items():
        ...
        if compname.find("ML") == 0:
            ...
            newcompname = "M" + FormatCorString(
                cor + getUIFODisp(comp.nodes[1].name[7])[0]
            )
        if compname[0] == "B":
        ...
        if compname.find("mUD") == 0:
            ...
            newcompname = (
                "SY"
                + FormatCorString(cor - [0, 1])
                + "__"
                + FormatCorString(cor - [0, 2])
            )
        if compname.find("mRL") == 0:
            cor = getUIFOCors(compname[4:7], gridsize)
            newcompname = (
                "SX"
                + FormatCorString(cor + [1, 0])
                + "__"
                + FormatCorString(cor + [2, 0])
            )
        if compname in (
            "MDet",
            "MFI1",
            ...
            cor = (
                getUIFOCors(comp.nodes[1].name[3:6], gridsize)
                + getUIFODisp(comp.nodes[1].name[7])[0]
            )
    ...
    kat_script = kat_script.replace(
        "nrNodes_4 nMrNodes4_laser", "nMrNodes4_laser nrNodes_4"
    )
    kat_script = kat_script.replace(
        "nrNodes_5 nMrNodes5_laser", "nMrNodes5_laser nrNodes_5"
    )
    newkat = finesse.kat()
    newkat.parse(kat_script, preserveConstants=True)
    return newkat
...
def KatCleanup(
    kat_, score_thresh=0, f_start=5, f_end=1e4, pt_no=10, fast=False, exclusionlist=[]
):
    ...
    freq, aligoSSens, lossPowerElements, lossPowerPD = compute_strain(
        freq_info, kataligo, fast
    )
    ...
    freq, sens_orig, lossPowerElements, lossPowerPD = compute_strain(
        freq_info, kattemp, fast
    )
    ...
    def check_importance(kat_, text):
        ...
        if fast:
            ...
            try:
                freq, SSens, lossPowerElements, lossPowerPD = compute_strain(
                    freq_info, kat_
                )
            except:
                return True
    ...
    for compname in kattemp.components:
        ...
        if (
            "beamSplitter" in str(type(comp)) or "mirror" in str(type(comp))
        ) and hasattr(comp, "L"):
            if comp.L.value != 0:
                old_L = comp.L.value
                old_T = comp.T.value
        ...
        if "space" in str(type(comp)) and compname != "SX98_99__99_99":
            ...
            kattemp.parse(
                "xaxis "
                + different_signal
                + f" f log {f_start:.2f} {f_end:.2f} {pt_no:d}"
            )
...
def compute_strain(freq_info, katALigoSens, fast=False):
    ...
    if not fast:
        ...
        RINinputnoise = (
            freq / freq
        ) * 4e-9  # 1/sqrt(Hz) taken from Phys. Rev. D 102, 062003
        RINoutputnoises = RINinputnoise * np.abs(
            outamptf["poutf1"] - outamptf["poutf2"]
        )

        freqinputnoise = (
            freq
        ) * 1e-8  # Hz/sqrt(Hz) taken from Phys. Rev. D 102, 062003
        freqoutputnoises = freqinputnoise * np.abs(
            outfreqtf["poutf1"] - outfreqtf["poutf2"]
        )
    ...
    pout_deg = np.abs(
        outALigo["poutf1"] - outALigo["poutf2"]
    )  # Demodulated power [W/???]
    ...
    #        time.sleep(0.1)
        ...
        SSens = (
            totalnoise / poutf_m
        )  # Differential length change sensitivity [1/sqrt(Hz)]
        SSens_quant = (
            qnoise / poutf_m
        )  # Differential length change quantum limited sensitivity [1/sqrt(Hz)]
    ...
    for key in katALigoSensWithDCPDs.nodes.getNodes():  # going over all nodes
        for comp in katALigoSensWithDCPDs.nodes.getNodeComponents(
            katALigoSensWithDCPDs.nodes.getNodes()[key]
        ):  # going over all component connected to the node
            if any(
                [
                    relevantcomp in str(type(comp))
                    for relevantcomp in ["mirror", "beamSplitter", "modulator"]
                ]
            ):  # If the component is m/bs/m put a dc pd measuring the power going into the node
                PDName = "poutdc_" + str(comp) + "_" + key
                PDNames.append(PDName)
                katALigoSensWithDCPDs.parse(
                    "pd0 " + PDName + " " + key + "*"
                )  # Put a DCPD in that node

                if "modulator" in str(
                    type(comp)
                ):  # get the transmission through the component if it is specified. Otherwise compute it from T+R+L=1.
                    Trans.append(1)
                else:
