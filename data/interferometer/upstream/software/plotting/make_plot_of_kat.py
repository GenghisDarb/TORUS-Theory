...
def main(argv):
    ...
    if simplify:
        ...
        if target == 0:
            ...
            pltname = "merger3"
        kat = KatCleanup(
            kat, threshold, f_start=f_start, f_end=f_end, pt_no=10, fast=simplifyfast
        )
        ...
        with open(simpfilename, "w") as f:
            ...
            consts4removal = [
                const
                for const in kat.constants
                if len(re.findall(const, katscript)) < 2
            ]
            ...
            katlines.append(
                "xaxis " + kat.xaxis.getFinesseText()[0].split(" ")[1] + " f log\n"
            )
    ...
    if parseUIFO:
        kat = ParseUIFOkat(
            kat, 6, ["BSfin", "L_LO", "BSDet", "DetLO"], old_new_dict
        )  # Comment out when parsing regular solutions
        #
        ...
        kat = FixDetectorCode(
            kat, "SArmU", "mAtArmU", old_new_dict=old_new_dict, replace_comp="M"
        )
        kat = FixDetectorCode(
            kat, "SArmR", "mAtArmR", old_new_dict=old_new_dict, replace_comp="M"
        )
        kat = FixDetectorCode(
            kat, "sToLO", "DetLO", old_new_dict=old_new_dict, replace_comp="L"
        )

    kat = FixDetectorCode(
        kat, "stoPD1", no_of_grid_points=-1, old_new_dict=old_new_dict
    )
    ...
    kat = FixDetectorCode(
        kat, "sToDet1", no_of_grid_points=-1, old_new_dict=old_new_dict
    )
    kat = FixDetectorCode(
        kat, "sToDet2", no_of_grid_points=-1, old_new_dict=old_new_dict
    )
    ...
    for detector_name in [
        comp_name for comp_name in kat.components if comp_name.find("P") == 0
    ]:
    ...
    for space_cors in spaces_cors_fitted:
        ...
        DrawSpace(
            space_cors,
            space_powers[space_cors["spacename"]],
            kat.components[space_cors["spacename"]].L.value,
            canvas,
            disablelabels,
        )
...
def make_plot_kat(kat_file, pdf_file):
    ...
    sys.argv = [
        "plotsolution_SQZ_filtered_vacuum_and_LO_wText.py",
        "-i",
        input_file,
        "-o",
        output_file,
