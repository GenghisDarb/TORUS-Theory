...
def get_loss_from_katstr(
    kat_string, freq_info, log_file="log.txt", use_classical_noise=True
):
    did_succeed = False
    error_count = 0
    loss = 666.4242
    while (not did_succeed) and (error_count < 5):
        try:
            (
                freq_aligo,
                strain_aligo,
                lossPowerElements_aligo,
                lossPowerPD_aligo,
                lossPowerCoating_aLIGO,
                totalnoise,
                total_signal,
            ) = compute_baseline_aligo(
                freq_info, use_classical_noise=use_classical_noise
            )
            ...
            (
                ...
                freq_info,
                kat_string=kat_string,
                use_classical_noise=use_classical_noise,
            )
            ...
            print_file(
                ["problem with get_loss_from_katstr in geo_aLIGO10.py_1, ", log_file],
                file=log_file,
            )
            time.sleep(0.1)
            error_count += 1
...
def get_loss_from_file(
    kat_file, freq_info, log_file="log.txt", use_classical_noise=True
):
    did_succeed = False
    ...
    while (not did_succeed) and (error_count < 5):
        if True:
            ...
            print_file(
                ["problem with get_loss_from_file in geo_aLIGO10.py_2, ", log_file],
                file=log_file,
            )
            time.sleep(0.1)
            error_count += 1
...
def get_loss_and_save_file(
    ...
    while (not did_succeed) and (error_count < 5):
        try:
            ...
            print_file(
                ["problem with get_loss_and_save_file, ", log_file], file=log_file
            )
            time.sleep(0.1)
            error_count += 1
    return loss
...
def compute_strain(
    ...
    if np.any(poutf_m == 0):
        ...
        print_file(
            ["problem with poutf_m in geo_aLIGO10.py, ", log_file], file=log_file
        )
        time.sleep(0.1)
    ...
    while did_succeed == False:
        try:
            ...
            print_file(
                ["problem with ComputeSpacePowers in geo_aLIGO10.py, ", log_file],
                file=log_file,
            )
            time.sleep(0.1)
    # time_diff=time.time()-curr_time
    ...
    mirrors = [
        comp
        for compname, comp in katALigoSens.components.items()
        if "mirror" in str(type(comp))
    ]
    beamsplitters = [
        comp
        for compname, comp in katALigoSens.components.items()
        if "beamSplitter" in str(type(comp))
    ]
    comps_powers = {}
    comp_coating_power = {}
    # time_diff=time.time()-curr_time
        ...
        for node in comp.nodes:
            ...
            for connected_comp in connected_comps[connected_comps != comp]:
                if "space" in str(type(connected_comp)):
                    comp_powers = np.append(
                        comp_powers, space_powers[connected_comp.name]
                    )
    ...
    loss_all_PowerPD = [
        100 * logistic.cdf(7500 * (poutdc - 10**-2)) for poutdc in all_poutdc
    ]
...
def compute_baseline_aligo(
    freq_info,
    baseline_file="geo_voyager_pure10.kat",
    log_file="log.txt",
    use_classical_noise=True,
):
    ...
    while (not did_succeed) and (error_count < 5):
        try:
            (
                ...
                freq_info, kat_string=readfile, use_classical_noise=use_classical_noise
            )
            did_succeed = True
        except:
            print("problem with compute_baseline_aligo in geo_aLIGO10.py_3")
            print_file(
                ["problem with compute_baseline_aligo in geo_aLIGO10.py_3, ", log_file],
                file=log_file,
            )
            time.sleep(0.1)
            error_count += 1
...
if __name__ == "__main__":
    ...
    (
        ...
        totalnoise,
        total_signal,
    ) = compute_baseline_aligo(freq_info)
    ...
    loss = get_loss_from_katstr(
        readfile, freq_info, log_file="log.txt", use_classical_noise=True
    )
    print(loss)
