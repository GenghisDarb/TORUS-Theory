...
def create_strains(target, best="", baseline="", curr_id=0, curr_loss=666):
    ...
    if target == 0:
        ...
        freq_info_curr = [
            800,
            3000,
            NUM_OF_SIMULATION_POINTS,
        ]  # neutron star merger expanded
        pltname = "large range"
    ...
    while not did_succeed:
        try:
            (
                ...
                freq_info, readfile, use_classical_noise=use_classical_noise
            )
            did_succeed = True
        except:
            time.sleep(0.1)
    ...
    while not did_succeed:
        try:
            (
                ...
                lossPowerPD_aligo,
                lossPowerCoating_aligo,
                totalnoise_aligo_full,
                totalsignal_aligo_full,
            ) = compute_baseline_aligo(
                freq_info_full, use_classical_noise=use_classical_noise
            )
            did_succeed = True
        except:
            time.sleep(0.1)
    ...
    while not did_succeed:
        try:
            (
                ...
                lossPowerPD,
                lossPowerCoating,
                totalnoise_best_full,
                totalsignal_best_full,
            ) = compute_strain(
                freq_info_full, readfile, use_classical_noise=use_classical_noise
            )
    ...
    if len(baseline) > 0:
        ...
        while not did_succeed:
            try:
                (
                    freq_baseline_full,
                    strain_baseline_full,
                    lossPowerElements,
                    lossPowerPD,
                    lossPowerCoating,
                    totalnoise_baseline_full,
                    totalsignal_baseline_full,
                ) = compute_strain(
                    freq_info_full, readfile, use_classical_noise=use_classical_noise
                )
                did_succeed = True
...
def plot_strain_sensitivity(target, input_file, baseline, new_file_dir="."):
    ...
    (
        ...
        totalnoise_baseline,
        totalsignal_baseline,
    ) = create_strains(target=target, best=input_file, baseline=baseline)
    ...
    ax2 = inset_axes(
        plt.gca(), width="30%", height="30%", loc="upper center"
...
if __name__ == "__main__":
    ...
    plot_strain_sensitivity(
        target=target, input_file="example_setup_type5.kat", baseline=baseline
    )
