...
For perfect closure without a separate controller, we would require
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.1626}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.4825}}
...
state \$\textbar m\textbackslash rangle\$ indicating how many
observations have been ``locked in'' (like memory of distinct events).
As the recursion proceeds, \$m\$ might increment. By 13D, let's say the
observer's state is \$\textbar m\_f\textbackslash rangle\$. Now 0D's
observer state was \$\textbar m\_i\textbackslash rangle\$. For true
closure, we require \$\textbar m\_f\textbackslash rangle\$ corresponds
to \$\textbar m\_i\textbackslash rangle\$ (perhaps \$m\_f = m\_i\$ in
modulo sense). The controller would facilitate this by an operation
\$\textbar m\_f\textbackslash rangle \textbackslash to
\textbar m\_i\textbackslash rangle\$. If \$m\_f - m\_i =
\textbackslash ell\$, the controller action could be conceptualized as
