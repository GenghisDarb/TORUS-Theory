...
def _prep(strain: np.ndarray, fs: float,
    ...
    try:
        ...
        if not (0 < low < 1 and 0 < high < 1):
            raise ValueError("Filter frequencies must be between 0 and Nyquist frequency.")
...
############################################
def torus_echo(strain: np.ndarray,
               fs: float,
               merger_time: float,
               tau_base: float,
               snr_thresh: float = 5.0) -> Dict[str, Any]:
    """
...
def thet_echo(strain: np.ndarray,
    ...
    template = np.zeros_like(w)
    if delay_idx < nt:
        template[delay_idx:] = np.exp(- (t[:nt-delay_idx] / 0.03)) \
                               * np.sin(2*np.pi*150*t[:nt-delay_idx])
