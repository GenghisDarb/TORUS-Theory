...
def main() -> None:
    ...
    parser.add_argument(
        "--dt", type=float, default=0.05, help="Nominal period of each tick, in seconds"
    )
    parser.add_argument(
        "--phase_jump",
        type=int,
    ...
    parser.add_argument(
        "--no_sleep", action="store_true", help="Run instantly without real-time delay"
    )
    ...
    lsb = [
        (obs[i + args.phase_jump][1] - obs[i][1] - args.phase_jump * args.dt) * 1e9
